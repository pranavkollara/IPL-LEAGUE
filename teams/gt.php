...
$play = "SELECT * from teams WHERE team LIKE 'gt';";
...
if (isset($_POST['submit'])) {
    ...
    if ($result = $conn->query($sql_query)) {
        while ($row = $result->fetch_assoc()) {
            ...
            $update = $played+1;
            $sql_update_team ="UPDATE `teams` SET `played` = '$update' WHERE `teams`.`team` = 'gt';";
...
echo "<h1 class='text-center my-4'>GT MATCH NO {$played}</h1>";
    ?>
    <div class=" container">
        <form action="gt.php" method="post">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php
                    require_once("../conn.php");
                    $sql_query = "SELECT * FROM `player_point_1` where team LIKE 'gt' ORDER BY `player_point_1`.`team` ASC;";
                    if ($result = $conn->query($sql_query)) {
                        while ($row = $result->fetch_assoc()) {
                            $player = $row['player'];
                            ...
                            <tr>
                                <th scope="row">
                                    ...
                                    if (file_exists("gt/$player.png")) {

                                        echo "<img style='margin-left:5px;width:10vw;min-width:50px' src='gt/$player.png'>";
                                    } else {
        ...
        <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <input name="team" id="team" class="d-none" value="gt"></input>
        <button type="submit" id="submit" name="submit" class="btn btn-primary">Submit</button>            
       </form>
    </div>
    <?php $team_team = 'gt' ?>      
