...
$play = "SELECT * from teams WHERE team LIKE 'csk';";
...
if (isset($_POST['submit'])) {
    ...
    if ($result = $conn->query($sql_query)) {
        while ($row = $result->fetch_assoc()) {
            ...
            $update = $played + 1;
            $sql_update_team = "UPDATE `teams` SET `played` = '$update' WHERE `teams`.`team` = 'csk';";
...
<body>
    ...
    <?php
    echo "<h1 class='text-center my-4'>CSK MATCH NO {$played}</h1>";
    ?>
    <div class=" container">
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <tr>
                                <th scope="row">
                                    ...
                                    if (file_exists("csk/$player.png")) {

                                        echo "<img style='margin-left:5px;width:10vw;min-width:50px' src='csk/$player.png'>";
                                    } else {
        ...
        <form method="post" action="team_edit.php">
            ...
            <input name="team" id="team" class="d-none" value="csk"></input>
            <button type="submit" id="submit" name="submit" class="btn btn-primary">Submit</button>
        </form>
    </div>
    <?php $team_team = 'csk' ?>      
