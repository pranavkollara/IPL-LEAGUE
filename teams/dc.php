...
$play = "SELECT * from teams WHERE team LIKE 'dc';";
...
if (isset($_POST['submit'])) {
    ...
    if ($result = $conn->query($sql_query)) {
        while ($row = $result->fetch_assoc()) {
            ...
            $update = $played+1;
            $sql_update_team ="UPDATE `teams` SET `played` = '$update' WHERE `teams`.`team` = 'dc';";
...
echo "<h1 class='text-center my-4'>DC MATCH NO {$played}</h1>";
    ?>
    <div class=" container">
        ...
        <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <input name="team" id="team" class="d-none" value="dc"></input>
        <button type="submit" id="submit" name="submit" class="btn btn-primary">Submit</button>            
       </form>
    </div>
    <?php $team_team = 'dc' ?>      
